...
#define EN_PIN 8
//#define RW_PIN 14 // it is connected to GND directly in Our panda Devkit.
#define RS_PIN 11
...
// 4. MACAL MACROs
#define LCD_PIN_HIGH(pin)   			HAL_GPIO_WritePin(GPIOA, pin,GPIO_PIN_SET) 
#define LCD_PIN_LOW(pin)   				HAL_GPIO_WritePin(GPIOA, pin,GPIO_PIN_RESET)
#define LCD_PORT_WRITE(data)			HAL_GPIO_WritePin(GPIOB, data,GPIO_PIN_SET)
...
#if MODE8_4 == 4
#define LCD_PORT_HIGH(data) 			LCD_PORT_RESET_4DATA();LCD_PORT_WRITE((data) & 0xf0);LCD_PIN_LOW(RS_PIN);lcd_ENABLE_R_W_DATA();LCD_PORT_RESET_4DATA();LCD_PORT_WRITE((char)((data)<<4));LCD_PIN_LOW(RS_PIN);
#define LCD_PORT_HIGH2(data) 			LCD_PORT_RESET_4DATA();LCD_PORT_WRITE((data) & 0xf0);LCD_PIN_HIGH(RS_PIN);lcd_ENABLE_R_W_DATA();LCD_PORT_RESET_4DATA();LCD_PORT_WRITE((char)((data)<<4));LCD_PIN_HIGH(RS_PIN);
#endif
