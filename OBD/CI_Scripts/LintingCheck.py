...
cpp_check_report= "cppcheck_report.csv"
cpp_check_path=0
...
def AdjustReport(outputReport):
 ...
 with open(outputReport, 'r') as input_file, open(temp_filename, 'w', newline='') as output_file:
        ...
        for row in reader:
            ...
            if '^' not in ''.join(row) and 'missingInclude' not in ''.join(row) :
...
def QualityGate1(outputReport,CppCheckQualityLimit):
    ...
    with open(outputReport, 'r') as report:
      ...
      if num_lines > CppCheckQualityLimit:
          print(f"Quality Gate Failed: Files have {num_lines} lint warnings (Quality Limit is {CppCheckQualityLimit})")
          exit(1)
      else:
